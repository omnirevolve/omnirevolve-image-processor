# 07_1_preview.py — preview after intra-layer dedup
# Inputs per layer (STRICT):
#   lines_intra.pkl
#   taps_intra.pkl
# Outputs:
#   <layer>/preview_intra.png
#   preview_intra_composite.png
...
def _palette(outdir: str, names: List[str], fallback: List[Tuple[int,int,int]]) -> Dict[str,Tuple[int,int,int]]:
...
def _load_lines(layer_dir: str) -> List[np.ndarray]:
    ...
    if not os.path.exists(p):
        print(f"[preview_intra] MISSING: {os.path.relpath(p, layer_dir)} — rendering without lines")
        return []
...
def _load_taps(layer_dir: str) -> List[Tuple[int,int]]:
    ...
    if not os.path.exists(p):
        print(f"[preview_intra] MISSING: {os.path.relpath(p, layer_dir)} — rendering without taps")
        return []
    with open(p,"rb") as f: obj=pickle.load(f)
...
def _draw_lines(polys: List[np.ndarray], size: Tuple[int,int], color: Tuple[int,int,int], th: int, aa: bool) -> np.ndarray:
    w,h = size; img = np.full((h,w,3),255,np.uint8)
...
def _draw_taps(img: np.ndarray, taps: List[Tuple[int,int]], color: Tuple[int,int,int], r: int):
    for (x,y) in taps:
...
def main():
    ...
    for name in cfg.color_names:
        ...
        # layer preview (lines black, taps red)
