# 10_preview.py — final preview (STRICT, uses simplified contours)
# Input per layer: contours_simplified.pkl
# Outputs: <layer>/preview_final.png, preview_final_composite.png
...
def main():
    ...
    for name in cfg.color_names:
        ...
        out_path = os.path.join(layer_dir, "preview_final.png")
        ...
        print(f"[preview_final] {name}: polylines={len(polys)} → {out_path}")

    comp = os.path.join(outdir, "preview_final_composite.png")
    cv2.imwrite(comp, composite)
    print(f"[preview_final] composite saved: {comp}")
