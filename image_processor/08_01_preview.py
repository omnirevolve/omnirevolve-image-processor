# 08_01_preview.py — preview after cross-layer dedup
# Inputs per layer (STRICT): lines_cross.pkl, taps_cross.pkl
# Outputs: <layer>/preview_cross.png, preview_cross_composite.png
...
def _load_lines(layer_dir:str)->List[np.ndarray]:
    p = os.path.join(layer_dir,"lines_cross.pkl")
    if not os.path.exists(p):
        print(f"[preview_cross] MISSING: {os.path.relpath(p, layer_dir)} — rendering without lines")
        return []
    with open(p,"rb") as f: obj=pickle.load(f)
    return obj if isinstance(obj,list) else []

def _load_taps(layer_dir:str)->List[Tuple[int,int]]:
    p = os.path.join(layer_dir,"taps_cross.pkl")
    if not os.path.exists(p):
        print(f"[preview_cross] MISSING: {os.path.relpath(p, layer_dir)} — rendering without taps")
        return []
    with open(p,"rb") as f: obj=pickle.load(f)
    taps=[]
    for it in obj:
...
def _draw_lines(polys,size,color,th,aa):
...
def _draw_taps(img,taps,color,r):
    for (x,y) in taps:
...
def main():
    ...
    for name in cfg.color_names:
        ...
        lines = _load_lines(layer_dir)
        taps  = _load_taps(layer_dir)

        lay = _draw_lines(lines, size, (0,0,0), th, aa)
        _draw_taps(lay, taps, (0,0,255), pen_r)
        out_l = os.path.join(layer_dir, "preview_cross.png")
        ...
        print(f"[preview_cross] {name}: lines={len(lines)}, taps={len(taps)} → {out_l}")

    out_c = os.path.join(outdir, "preview_cross_composite.png")
    cv2.imwrite(out_c, composite)
    print(f"[preview_cross] composite saved: {out_c}")
