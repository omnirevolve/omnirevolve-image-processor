# 09_scale_vector.py  (only changes: scale taps -> taps_final.pkl)
import os
import json
import pickle
import numpy as np
...
def export_to_svg(contours, color_name: str, color_rgb: Tuple[int,int,int],
    ...
    lines.append('</g>')
    lines.append('</svg>')
    with open(os.path.join(cfg.output_dir, color_name, "output.svg"), "w") as f:
        f.write("\n".join(lines))
...
def scale_all_vectors(cfg: Config):
    ...
    for i, color_name in enumerate(cfg.color_names):
        ...
        with open(os.path.join(base, "contours_simplified.pkl"), "rb") as f:
        ...
        with open(os.path.join(base, "contours_final.pkl"), "wb") as f:
            pickle.dump(scaled_contours, f)

        # scale taps if present
        taps_path = os.path.join(base, "taps.pkl")
        if os.path.exists(taps_path):
            with open(taps_path, "rb") as f:
                taps = pickle.load(f)  # Nx2 float32
            taps_scaled = (taps * scale).astype(np.float32)
            with open(os.path.join(base, "taps_final.pkl"), "wb") as f:
                pickle.dump(taps_scaled, f)

        bgr = cfg.colors[i]; rgb = (bgr[2], bgr[1], bgr[0])
